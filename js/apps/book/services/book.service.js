...
function addReview(book, review) {
    if (!book) return;
    review.userName = review.userName ? review.userName : 'Books Reader';
    review.readAt = review.readAt ? review.readAt : new Date().toLocaleString();
    review.rating = (!review.rating) ? '⭐' : '⭐'.repeat(review.rating);
    review.txt = review.txt ? review.txt : 'No content';
    review.id = utilService.makeId();

    const bookId = book.id;
    const bookIdx = gBooks.findIndex(book => book.id === bookId)

    if (!gBooks[bookIdx].reviews) gBooks[bookIdx].reviews = [review];
    else gBooks[bookIdx].reviews.push(review);
}
...
function getNextBookId(bookId, diff) {
    const bookIdx = gBooks.findIndex(book => book.id === bookId);
    let nextBookId = bookIdx + diff;
    if (nextBookId === gBooks.length) nextBookId = 0;
    else if (nextBookId < 0) nextBookId = gBooks.length - 1;
    return gBooks[nextBookId].id;
}
...
const gSearchedNames = storageService.loadFromStorage('searchedNamesDB') || {};

function getTopResults(searchName) {
    if (!searchName) return;
    if (gSearchedNames[searchName]) {
        return Promise.resolve(gSearchedNames[searchName]);
    }

    const searchStr = searchName.replace(/\s/g, '%20')
    const url = `https://www.googleapis.com/books/v1/volumes?printType=books&q=${searchStr}`
    return axios.get(url)
        .then((res) => {
            gSearchedNames[searchName] = res.data.items;
            storageService.saveToStorage('searchedNamesDB', gSearchedNames);
